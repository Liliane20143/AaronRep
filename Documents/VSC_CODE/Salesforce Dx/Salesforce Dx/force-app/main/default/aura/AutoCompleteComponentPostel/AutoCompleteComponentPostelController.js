({ 
	   ...
	   if(countryinput == undefined || countryinput == null || countryinput == '' || countryinput == 'ITALIA'){
		...
		if(isPV != 'true'){
			if(countryinput != 'ITALIA'){
				...
				addressMapping.streetnumberdisabled = 'false'; //gianluigi.virga 10/09/2019 - UX-55 - streetnumber always active
				component.set("v.addressMapping", addressMapping);
			}
		}
		...
		if(countryinput != 'ITALIA'){
			...
			component.set("v.isEE",false);
			component.set("v.addressMapping.zipcodedisabled",'false'); //gianluigi.virga 09/09/2019
		}
		...
		/* START 	Doris D.       21/02/2019 ---------------*/
		//NEXI-348 Marta Stempien <marta.stempien@accenture.com 24/09/2019 Start
		component.set("v.isChecked", true);
		helper.checkboxChanged( component, true );
		//NEXI-348 Marta Stempien <marta.stempien@accenture.com 24/09/2019 Stop
		component.set( "v.isLoaded", true );

		/* END   	Doris D.       21/02/2019 ---------------*/
	 },
	...
	handleShowModalEvent : function(component, event, helper) {
		...
		if(countrySel != 'ITALIA'){
			...
			addressMapping.districtdisabled = 'true';
			addressMapping.streetdisabled = 'true';
			addressMapping.streetnumberdisabled = 'false'; //START gianluigi.virga - 10/09/2019 - UX-55
			component.set("v.addressMapping", addressMapping);
		}
		...
		if(objectDataMap.setRedBordercompanyData == true){
			...
			console.log("mandatory field from map: " + JSON.stringify(mapFromNext));
				...
				for (var keys in mapFromNext)
				{
					...
					// Doris dongmo, 17/05/2019 - ADDED TOAST TO INSERT ADDRESS - START
					try{
						if( keys.includes('zipcodelegaleRappresentante') || keys.includes('civicolegaleRappresentante') 
						    || keys.includes('stradalegaleRappresentante') || keys.includes('comunelegaleRappresentante') ){
							var toastEvent = $A.get("e.force:showToast");
							toastEvent.setParams({
								title :  $A.get("$Label.c.OB_CustomErrorLabelTech"),
								message: $A.get("$Label.c.OB_InsertValidAddressForExecutor"),
								duration:' 10000',
								key: 'info_alt',
								type: 'error',
								mode: 'pester'
					...
					// Doris dongmo, 17/05/2019 -  ADDED TOAST TO INSERT ADDRESS - END
	...
	{
	   helper.checkboxChanged(component, event.getSource().get("v.checked")); //NEXI-348 Marta Stempien <marta.stempien@accenture.com> 24/09/2019 Method content moved to helper.checkboxChanged
	}
})