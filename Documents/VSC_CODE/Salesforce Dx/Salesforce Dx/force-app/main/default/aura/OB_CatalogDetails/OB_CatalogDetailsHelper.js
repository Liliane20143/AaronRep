        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                for(var j = 0; j<parents.length; j++){
                	var cloneId = '';
                	/*if(clonedRows != undefined){
                		for(var y = 0; y<clonedRows.length; y++){
                			console.log("inside clonewRows for...");
                			console.log("clone id"+ clonedRows[y].Id);
		                	if(parents[j].Id == clonedRows[y].Id){
		                		cloneId = parentCloneId;
		                		break;
		                	}else{
		                		cloneId = '';
		                	}
  ...
  /*  handleRowsAttributes: function(component, event) {
   ...
   /* getCatalogItems: function(component, event) {
    ...
    deleteRowServer: function(component, event, selectedRow) {
	 ...
	 createProductsList: function(component, event, rows) {
		 var prodIdList = [];
		 for(var i = 0; i<rows.length; i++){
	 ...
	 fetchCatalogItemsByProductsId: function(component, event, prodIdList) {
