({ 
	 doInit : function(component, event, helper) {
		...
		helper.loadConfigurazioni(component, page);// add 'page' ....Doris D. 21/03/2019	    
	},
	...
	onPagePrevious: function(component, event, helper) {
		...
		var from = document.getElementById("dateFromNEXI").value;
    ...
    onPageNext: function(component, event, helper) {
    	...
    	var to = document.getElementById("dateTo").value;
        if(!$A.util.isEmpty(name)|| !$A.util.isEmpty(from) || !$A.util.isEmpty(to) || !$A.util.isEmpty(listino) || !$A.util.isEmpty(schemaPrezzi)  || !$A.util.isEmpty(modello)  ){
    		console.log("calling searchconfigurazioni");
    		helper.searchconfigurazioni(component,page);
    	}
        else{
        	console.log("calling loadConfigurazioni");
        	helper.loadConfigurazioni(component,page);
        }		
    },
    ...
    searchConfigurazioni :function(component, event, helper) {
    	...
    	component.set("v.nameProduct",name);
    	...
    	document.getElementById("dateFromNEXI").getElementsByTagName("input")[0].classList.remove("borderError");
    ...
    newConfigurazione:function(component, event, helper) {
        ...
        var name = component.get("v.nameProduct");//var name = component.get("v.name"); // Doris D. 15/03/2019
        var lookupInput = component.find("lookupCatalogInputId");
		...
		//	component.set("v.name",'');// COMMENT Doris D. 15/03/2019
			...
			component.set("v.nameProduct",[]);//add by  Doris D. 15/03/2019
	    	...
	    	document.getElementById("dateFromNEXI").value= '';
			...
			*/
    		...
    		if(from == '')
    		{
    			document.getElementById("dateFromNEXI").getElementsByTagName("input")[0].classList.add("borderError");
    			//add error msg
    			errorMsgs.push({
    				parameter: "dateFrom",
	...
	goBackToOffertaCatalogBank: function(component, event, helper) { 
		component.set("v.goBackToABIoffers", true);
	/*	var selectedABI = component.get("v.selectedABI");
		var event = $A.get("e.force:navigateToComponent");
	    event.setParams({
	        componentDef : "c:OB_ABI_Offers", //Nexi per Banca
	        componentAttributes: {
	            selectedRecord: selectedABI,
