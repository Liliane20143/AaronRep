        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
              //  console.log("From server retrieveMatrixParameterRows: ", response.getReturnValue());
                console.log("From server retrieveMatrixParameterRows: ", response.getReturnValue().length);
                var  rows= response.getReturnValue();
                component.set("v.rowsFromServer", rows);
                //this.createProductsList(component, event, rows);
  ...
  /*  handleRowsAttributes: function(component, event) {
    ...
    cloneRow: function(component, rowToClone, index, event) {
     ...
     //  parentChildrenRows.splice(index, 0, clonedRows.parent);
        ...
        */
        //END JS PART
        var catalogItemsToClone =  component.get("v.catalogItemsToClone");
		...
		var action = component.get("c.insertCloneRow");
 		action.setParams({
            ...
            catalogItemsToClone: catalogItemsToClone
        });
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
    ...
    saveRows: function(component, event) {
        ...
        console.log("into saveRows - rows: ", rows);
   ...
   /* getCatalogItems: function(component, event) {
    ...
    insertNewRows: function(component, event) {
        ...
        var matrixParameterId = component.get("v.matrixParameter").Id;
         var matrixParameter = component.get("v.matrixParameter");
         console.log("@@@matrixParameter: " , matrixParameter);
    ...
    getFamilies: function(component, event, productId) {
        console.log("into getFamilies");
        console.log("productId into getFamilies: " + productId);
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                for (var i = 0; i < productFamilies.length; i++) {
                	//var tempFam = {'sobjectType':'NE__family__c', 'Id': productFamilies[i].NE__FamilyId__c};
                   var tempFam = productFamilies[i].NE__FamilyId__c;
                  /*  families.push({
                        "Id": productFamilies[i].NE__FamilyId__c  
                    });*/
                    families.push(tempFam);
                }
    ...
    deleteRowServer: function(component, event, selectedRow) {
	 ...
	 createProductsList: function(component, event) {
		 var prodIdList = [];
		 var rows = component.get("v.rowsFromServer"); 
		 for(var i = 0; i<rows.length; i++){
			 prodIdList.push({"Id" : rows[i].OB_Product__c});
		 }
		 component.set("v.prodIdList", prodIdList);
		 console.log("prodIdList: ", prodIdList);
		 this.fetchCatalogItemsByProductsId(component, event, prodIdList);
	 },
	 
	 fetchCatalogItemsByProductsId: function(component, event) {
        console.log("into fetchCatalogItemsByProductsId");
        var prodIdList = component.get("v.prodIdList");
        var productName = component.get("v.matrixParameter").OB_Componente__r.NE__Item_Header__r.Name;
        console.log("productName: " + productName);
        var action = component.get("c.fetchCatalogItemsByProductIdServer");
        action.setParams({
            productName: productName,
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                console.log("catalogItems From server: ", response.getReturnValue());
                component.set("v.catalogItemsToClone", response.getReturnValue());
            } else if (state === "INCOMPLETE") {
                // do something
            } else if (state === "ERROR") {
                var errors = response.getError();
