...
rule stepToGo {
	...
	when{
		f1 : FieldProps f1.objectsData.unbind.goToNextStepSocieta =='goToSocieta';
	}
	then{
		var methodArguments= {};
...
rule showVat {
	agenda-group:'Dati_operativi_4';
	when{
		f1 : FieldProps f1.objectsData.unbind.VAT_notPresent_Main =='true';
	}
	then{
		var metadataToUpdate=[];
...
rule Show_VATNotPresent {
	agenda-group:'Dati_operativi_4';
	when{
		f1 : FieldProps f1.objectsData.unbind.VAT_notPresent_Main =='false';
	}
	then{
		var metadataToUpdate=[];
		...
		metadataToUpdate.push({path:"Dati_societa_5.merchant_NE__VAT__c",property:"required",value:false});
		metadataToUpdate.push({path:"Dati_societa_5.merchant_NE__VAT__c",property:"disabled",value:true});
		emit("updateInptCmpsMetadata",metadataToUpdate);
	}
}
rule automaticShowLegalForm {
	agenda-group:'Dati_operativi_4';
	when{
		f1 : FieldProps f1.objectsData.unbind.OB_Legal_Form_Main =='NOPROFIT';
	}
	then{
		var metadataToUpdate=[];
		metadataToUpdate.push({path:"Dati_societa_5.merchant_OB_No_Profit_Recipient_Class__c",property:"hidden",value:false});
...
rule makeYearCompanyReadOnly {
	agenda-group:'Dati_operativi_4';
	when{
		f1 : FieldProps (f1.objectsData.merchant.hasOwnProperty("OB_Year_constitution_company__c") && f1.objectsData.merchant.OB_Year_constitution_company__c >0 ) ;
...
rule HiddenOpeningWeeklyInMaintenance {
	agenda-group:'Punto_Vendita_9';
	when{
...
rule testCoherencyOnStartSeasonal {
	agenda-group:'Punto_Vendita_9';
	when{
...
rule testCoherencyOnEndSeasonal {
	agenda-group:'Punto_Vendita_9';
	when{
...
rule goToFinalStep {
	agenda-group:'Dati_operativi_4';
	when{
		f1 : FieldProps f1.objectsData.unbind.goToFinalStep =='true';
	}
	then{
		var methodArguments= {};
